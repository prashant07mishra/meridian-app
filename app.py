...
mail_receiver = os.getenv("MAIL_RECEIVER", mail_user)  # Defaults to mail_user if not set
...
app.config["MAIL_DEFAULT_SENDER"] = ("Meridian Inquiries", mail_user)
...
def index():
    if request.method == "POST":
        ...
        # 2. Guard against missing receiver before sending
        recipient_target = os.getenv("MAIL_RECEIVER") or os.getenv("MAIL_USERNAME")
        
        if not recipient_target or not mail_pass:
            print("ERROR: Email credentials missing in environment variables!")
            flash(f"Thank you, {name}. Your inquiry was recorded.", "info")
